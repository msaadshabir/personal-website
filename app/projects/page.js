...
const ProjectCard = memo(({ project }) => (
  <div className="space-y-3">
    ...
    <a
      ...
      className="text-xl md:text-2xl font-bold text-zinc-900 dark:text-zinc-50 hover:text-zinc-800 dark:hover:text-zinc-200 transition-colors duration-200 inline-block"
    >
      {project.name}
    </a>
    ...
    <div className="flex flex-wrap gap-2">
      {project.tags.map((tag) => (
        <span
          key={tag}
          className="px-3 py-1.5 text-sm font-medium bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 rounded-full hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors cursor-default"
        >
          {tag}
        </span>
      ))}
